...
| [OmniQuant_AI_Handover.md](OmniQuant_AI_Handover.md) | **OmniQuant AI** — quantitative recovery analytics: XGBoost propensity, Kaplan-Meier/Cox survival, fuzzy-matching reconciliation, causal impact testing | [omniquant-ai](https://github.com/kristic8998/omniquant-ai) (private) | v1.0.0, 84 tests, CI green |

## Honest scope note (read this first)

**Update 2026-07-28:** OmniQuant AI added, written **first-hand**. Two things in it are worth borrowing elsewhere. First, `app/modules/catalog.py` is a single declarative source for every module's required columns, prerequisites, workflow steps, per-button explanations and statistical caveats; the Help screen and each module page render *from* it, so on-screen documentation cannot drift from the code. Second, `mock_data_simulator.py` writes a `GROUND_TRUTH.txt` alongside its output, which turns "looks plausible" into a real accuracy test — the survival model is verified to land within 1% of a planted median while a censoring-naive estimate is 38% optimistic. Its GUI, like FinOps Command Center's, has never been rendered on a real screen.
