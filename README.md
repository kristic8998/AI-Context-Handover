...
| [FinOps_Command_Center_Handover.md](FinOps_Command_Center_Handover.md) | **FinOps Command Center** — the application containing both modules below (shell, threading, packaging, CI) | [finops-command-center](https://github.com/kristic8998/finops-command-center) (private) | v1.0.0, CI green |
...
**Update 2026-07-26:** DocuParse_AI and Cronus_Orchestrator were initially omitted because no standalone repos by those names exist. The owner has since provided the build session's own handoff records (`SESSION_HANDOFF.md` + `FILE_MAP.md`) showing both exist as **modules inside the private `finops-command-center` repo**, built in a parallel session. Their handover docs are now included, **derived from those records rather than a first-hand code read** — each doc states this provenance in its header; treat the code as authoritative where they disagree. Note that FinOps Command Center’s GUI has never been rendered on a real screen (verified only via a mock-Tk layer); see the 2026-07-27 update below for what has since changed.

**Update 2026-07-27:** FinOps Command Center now has a committed test suite (**156 pytest tests**, synthetic fixtures only) and **green CI** — lint, a four-way matrix (Ubuntu + Windows × Python 3.11 + 3.12), a packaging-sanity job that fails when a lazily-imported package is missing from `build.spec` hidden imports, and an advisory forward-compatibility job. It also no longer contains MIS Studio: that feature set belongs to `finsight`, and keeping both would have let the same portfolio age differently depending on which tool ran. [FinOps_Command_Center_Handover.md](FinOps_Command_Center_Handover.md) is written **first-hand** by the architect who built the app and supersedes the two module docs wherever they disagree — including the flat `app/` package (not `src/`) and the Python 3.11 floor. Its GUI remains unverified on a real screen.
